 ...
 async handleStopRecording() {
  ...
  try {
    ...
    if (chat.type === "user") {
      ...
      // 1. Enviar audio por ICE
      success = await window.IceDelegate.sendAudioMessage(
        user.username,
        audioData.pcm16
      );
      ...
      setTimeout(async () => {
        await this.loadMessages("user", chat.id);
      }, 500);
    ...
    // =========================
      ...
      // ✅ CREAR/ASEGURAR GRUPO EN ICE PRIMERO
      const memberUsernames = (group.memberIds || group.members || [])
        ...
        console.log(`📨 Asegurando grupo ICE con miembros:`, memberUsernames);

        // ID único y estable para el grupo en ICE
        const iceGroupId = `group_${group.id}`;

        console.log(`🆔 ICE Group ID: ${iceGroupId}`);

// Crear o reutilizar el grupo de llamada
      await window.IceDelegate.joinMessagingGroup(iceGroupId, memberUsernames);
      ...
      console.log(`✅ Grupo ICE confirmado: ${iceGroupId}`);
      ...
      try {
        ...
        console.error("❌ Error guardando audio grupal:", err);
      }
